...
class CoverageCheckServer : public rclcpp::Node
{
private:
	// callback function for the server
	void checkCoverageCallback(const std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Request> request,
...
protected:
	...
	// ros2 server object
	rclcpp::Service<ipa_building_msgs::srv::CheckCoverage>::SharedPtr coverage_check_server_;

public:
	// constructor
	CoverageCheckServer(bool en_coverage_check_server = false);
	// CoverageCheckServer(const rclcpp::NodeOptions &options);

	bool checkCoverage(const std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Request> request,
					   std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Response> response);
