...
CoverageCheckServer::CoverageCheckServer(bool en_coverage_check_server) : rclcpp::Node("coverage_check_server", rclcpp::NodeOptions())
{
	if (en_coverage_check_server)
	{
		coverage_check_server_ = this->create_service<ipa_building_msgs::srv::CheckCoverage>("coverage_check", std::bind(&CoverageCheckServer::checkCoverageCallback, this, std::placeholders::_1, std::placeholders::_2));
		RCLCPP_INFO(this->get_logger(), "Server for coverage checking initialized.....");
	}
}

void CoverageCheckServer::checkCoverageCallback(const std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Request> request,
												std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Response> response)
{
	checkCoverage(request, response);
}

bool CoverageCheckServer::checkCoverage(const std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Request> request,
										std::shared_ptr<ipa_building_msgs::srv::CheckCoverage::Response> response)
{
										...
										const bool check_for_footprint, const bool check_number_of_coverages, cv::Mat &coverage_map, cv::Mat &number_of_coverage_image)
{
	...
	if (check_number_of_coverages == true)
	{
		...
		std::cout << "Checking number of coverages." << std::endl;
	}
